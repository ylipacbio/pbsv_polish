...
def get_parser():
    ...
    parser = ArgumentParser(
        "Extract a substring (chrom, start, end) from input FASTA file and save to output FASTA file.")
...
def run(args):
    ...
    substr_fasta(fileobj=fileobj, chrom=str(args.chrom), start=int(
        args.start), end=int(args.end), out_fa_fn=args.output_fa_fn)
